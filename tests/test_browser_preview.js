...
assert.ok(js.includes('keepalive: true'));
assert.ok(js.includes('getRandomValues'));
assert.ok(js.includes('pagehide'));
assert.ok(js.includes("parsed.searchParams.set('session', token)"));
assert.ok(cpp.includes('resolvePrivatePreviewCloseTarget'));
assert.ok(cpp.includes('privatePreviewTokenFromTarget'));
assert.ok(streamManager.includes('closePreviewSession('));
assert.ok(streamManager.includes('cancelledPreviewSessions'));
assert.ok(streamManager.includes('::shutdown(session.upstreamFd, SHUT_RDWR)'));
...
console.log('PASS: 203.68 private preview close handshake, reconnect-race guard, relay fd shutdown, idle gate and embedded JS match');
